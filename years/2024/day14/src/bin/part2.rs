...
fn main() {
    ...
    for i in 0..10000 {
        ...
        let mut display: Vec<String> =
            vec![(" ".repeat(bbox.x_max as usize + 1)).to_string(); bbox.y_max as usize + 1];
        for j in 0..robots.len() {
            let robot = &robots[j];
            display[robot.pos.y as usize]
                .replace_range(robot.pos.x as usize..robot.pos.x as usize + 1, "#");
        }
        eprintln!("seconds elapsed: {}", i+1);
        for line in display {
            eprintln!("{}", &line);
        }
