    ...
    fn valid(&self) -> bool {
        ...
        fn combine(goal: u64, acc: u64, op: Op, mut operands: Vec<u64>) -> bool {
            ...
            let new_acc = match op {
                ...
                Op::CONCAT => {
                    let mut acc_s = acc.to_string();
                    acc_s.push_str(&operands.remove(0).to_string());
                    acc_s.parse().unwrap()
                }
            };
            ...
            return combine(goal, new_acc, Op::ADD, operands.clone())
                ...
                || combine(goal, new_acc, Op::CONCAT, operands.clone());
        }
        ...
        combine(self.goal, acc, Op::ADD, operands.clone())
            ...
            || combine(self.goal, acc, Op::CONCAT, operands.clone())
    }
}
